...
public class ReceiptDialog {
    ...
    // Fungsi Bantuan untuk rata kiri-kanan Bluetooth Printer
    ...
    private static void exportReceiptToPDF(Context context, TransactionModel tx, String displayInvoiceId, int totalQty, String shopName, String shopAddress, String shopFooter, boolean removeWatermark) {
        ...
        html.append("body { font-family: 'Courier New', Courier, monospace; font-size: 13px; padding: 20px; color: #000; width: 320px; margin: 0 auto; }");
        html.append(".center { text-align: center; }");
        html.append(".bold { font-weight: bold; }");
        html.append(".dashed { border-bottom: 1px dashed #000; margin: 10px 0; }");
        html.append(".row { display: table; width: 100%; }");
        html.append(".left { display: table-cell; text-align: left; }");
        html.append(".right { display: table-cell; text-align: right; }");
        ...
        if (tx.getItems() != null) {
            for (MenuModel item : tx.getItems()) {
                ...
                html.append("<div class='left'>&nbsp;&nbsp;").append(item.getStock()).append("x ").append(formatRupiah.format(item.getPrice()).replace("Rp", "")).append("</div>");
                html.append("<div class='right'>").append(formatRupiah.format(item.getPrice() * item.getStock()).replace("Rp", "")).append("</div>");
                html.append("</div>");
            }
        }
        html.append("<div class='dashed'></div>");
